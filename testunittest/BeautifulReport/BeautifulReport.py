...
SEND_KEY = "3208-df93241016cd006bcf70fa431f27f732"
API_ADDR = "https://pushbear.ftqq.com/sub?sendkey="+SEND_KEY
...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        error_log += "| CaseName      | status      | Log   |\n"
        error_log += "| ------------- |-------------| -----|\n"
        FIELDS['testResult'] = []
        for item in self.result_list:
            item = json.loads(str(MakeResultJson(item)))
            FIELDS.get('testResult').append(item)
            error_log += "| %s " % item.get("methodName")
            error_log += "| %s " % item.get("status")
            error_log += "| %s " % "More info please check email"#item.get("log")
            error_log += "|\n"
        ...
        self.FIELDS = FIELDS
        #if (FIELDS['testPass'] + FIELDS['testSkip']) != FIELDS['testAll']:
        # desp = "#Firmware autotest:" + " \n" \
        # + "##All case count:" + str(FIELDS['testAll']) + "\n"\
        # + "+ Pass count:" + str(FIELDS['testPass']) + "\n" \
        # + "+ Skip count:" + str(FIELDS['testSkip']) + "\n"\
        # + "+ Failed count:" + str(FIELDS['testFail']) + "\n" \
        # + "+ Error count:" + str(FIELDS['testError'])
        desp += "| testPass | testSkip  |  testFail | testError | testAll |\n"
        desp +="| ---------|---------- |-----------|-----------|---------|\n"
        desp += "| %d " % FIELDS['testPass']
        desp += "| %d " % FIELDS['testSkip']
        desp += "| %d " % FIELDS['testFail']
        desp += "| %d " % FIELDS['testError']
        desp += "| %d " % FIELDS['testAll']
        desp += "|\n"
        desp += "\n\n"
        desp += error_log
        ...
        urllib.request.urlopen(
            API_ADDR + "&text=" + urllib.parse.quote("Firmware autotest report "+ now) + "&desp=" + urllib.parse.quote(
                desp.encode('utf-8')), context=context)
        return FIELDS
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def __init__(self, suites,report_path=""):
        ...
        if report_path != "":
            self.config_tmp_path = report_path
    ...
    def report(self, description, filename: str = None, log_path='.'):
        ...
        self.output_report()
        text = '\nAll test case finished, you can go {} look the report'.format(self.log_path)
        print(text)
